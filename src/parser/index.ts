        ...
        switch (tokens[x].type) {
            case TokenType.FUNCTION: {
                ...
                for (let y = insideIndex; y < tokens.length; y++) {
                    ...
                    } else if (tokens[y].type === TokenType.BRACE_OPEN) {
                        braceCount++;
                    }
                    insideIndex++;
            ...
            case TokenType.VARIABLE_DECLARATION: {
                const variableType =
                    tokens[x].value === Symbols.UNALTERABLE_VARIABLE_DECLARATION
                        ? "unalterable"
                        : tokens[x].value ===
                          Symbols.ALTERABLE_VARIABLE_DECLARATION
                        ? "alterable"
                        : "";
                if (variableType === "")
                ...
                if (tokens[x + 1].type !== TokenType.LITERAL)
                    throw new Error("Invalid variable declaration");

                newVariable.name = tokens[x + 1].value;
                if (tokens[x + 2].type !== TokenType.ASSIGNMENT)
                    throw new Error("Invalid variable declaration assignment");
